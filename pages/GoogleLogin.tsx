...
const GoogleLogin: React.FC<GoogleLoginProps> = ({ onBack, onLogin }) => {
    const { t } = useUser();
    return (
        <div className="relative flex h-auto min-h-screen w-full flex-col bg-background-light dark:bg-background-dark group/design-root overflow-x-hidden font-display text-gray-900 dark:text-white transition-colors duration-200">
            ...
            <div className="flex-1 flex flex-col px-5 pb-8">
                ...
                <div className="flex flex-col gap-3 mt-8">
                    <button
                        onClick={onLogin}
                        className="w-full rounded-xl bg-white text-gray-800 border border-gray-200 dark:border-transparent font-bold text-base py-3.5 flex items-center justify-center gap-3 transition-transform active:scale-[0.99] shadow-lg shadow-black/5 hover:bg-gray-50"
                    >
                        <span className="material-symbols-outlined">account_circle</span>
                        <span>{t('googleLogin.continueWithGoogle')}</span>
                    </button>
                    <button
                        onClick={onBack}
